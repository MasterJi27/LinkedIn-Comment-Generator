...
document.addEventListener('DOMContentLoaded', function() {
    ...
    function updatePostPreview(content, caption) {
        ...
        if (!content) {
            ...
            noPost.textContent = 'Select a post to generate a comment';
            postContent.appendChild(noPost);
            return;
        }
        ...
        if (caption) {
            ...
            postContent.appendChild(captionEl);
        }

        const contentEl = document.createElement('div');
        contentEl.className = 'content';
        contentEl.textContent = content.length > 200 ? content.substring(0, 200) + '...' : content;
        postContent.appendChild(contentEl);
    }
    ...
    if (confirmBtn) confirmBtn.addEventListener('click', copyCommentToClipboard);
    if (hintInput) {
